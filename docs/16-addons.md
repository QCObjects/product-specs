...
| `QCObjects/qcobjects-openai-api` (private repo, members only) | AI proxy (`qcobjects-handler` + `qcobjects-api`, `v1.0.14`) | OpenAI chat-completions via the secret-hiding proxy pattern: browser `Service` → same-origin route → `BackendMicroservice` + `serviceLoaderNode` with server-side key. Ships browser UI component/controller + Node services + per-subpath `.cts/.mts/.ts` triplets | stable |
| `QCObjects/qcobjects-azure-openai-api` (private repo, members only) | AI proxy (`qcobjects-handler` + `qcobjects-api`, `v1.0.22`) | Same proxy architecture against Azure OpenAI endpoints (sibling of `qcobjects-openai-api`) | stable |
